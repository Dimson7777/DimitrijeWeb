 ...
 */
export const StatusCapsule = ({ label, className = "" }: StatusCapsuleProps) => {
  ...
  return (
    <motion.div
      ...
      className={`group relative inline-flex items-center gap-2.5 rounded-full border border-primary/30 bg-card/65 px-[1.125rem] py-2 text-[0.68rem] font-mono uppercase tracking-[0.15em] text-foreground/90 shadow-[0_10px_28px_-18px_hsl(var(--primary)/0.7)] sm:text-[0.7rem] ${className}`}
    >
      ...
      {/* Glowing dot — fixed footprint, centered with the text. The pulsing
          halo is absolutely positioned so it never shifts layout. */}
